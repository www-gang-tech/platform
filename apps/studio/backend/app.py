...
def _editor_auth_token():
    return os.environ.get('GANG_EDITOR_TOKEN') or os.environ.get('EDITOR_AUTH_TOKEN')


def _is_editor_authorized():
    expected_token = _editor_auth_token()
    if expected_token:
        auth_header = request.headers.get('Authorization', '')
        return (
            auth_header == f'Bearer {expected_token}'
            or request.headers.get('X-Editor-Token') == expected_token
        )
    return os.environ.get('EDITOR_MODE', '').lower() == 'true'


def _resolve_content_file(file_path: str) -> Path:
...
def auth_status():
    """Check whether the current request can edit content."""
    authenticated = _is_editor_authorized()
...
def get_content(file_path):
    ...
    try:
        full_path = _resolve_content_file(file_path)
...
def validate_headings():
    ...
    for i in range(1, len(heading_levels)):
        ...
        if curr_level > prev_level + 1:
            ...
            suggestions.append(
                'Increment heading levels by one (use H'
                + str(prev_level + 1)
                + ' instead of H'
                + str(curr_level)
                + ')'

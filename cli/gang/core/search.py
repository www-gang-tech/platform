...
class SearchIndexer:
    ...
    def generate_search_page_html(self) -> str:
        ...
        lang = html.escape(str(site.get('language', 'en')))
        site_title = html.escape(str(site.get('title', 'GANG')))
        description = html.escape(f"Search content on {site.get('title', 'GANG')}")
        canonical_url = html.escape(f"{str(site.get('url', '')).rstrip('/')}/search/")

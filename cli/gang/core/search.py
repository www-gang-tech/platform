...
class SearchIndexer:
    ...
    def generate_search_page_html(self) -> str:
        ...
        site_title = str(site.get('title', 'GANG'))
        site_url = str(site.get('url', '')).rstrip('/')
        canonical_url = f"{site_url}/search/" if site_url else "/search/"
        description = "Search articles, projects, pages, and people on this site."
        jsonld = json.dumps({
...
<body>
    ...
    <div class="search-box">
        <label for="searchInput">Search this site</label>
        <input 
            type="text" 
            id="searchInput" 
    ...
    <script>
        ...
        return (html
                ...
                .replace('__CANONICAL_URL__', escape(canonical_url))

...
class ContractValidator:
    ...
    def _configured_rule_names(self, section: str) -> List[str]:
        names = []
    ...
    def check_accessibility(self, html: str) -> List[Dict]:
        ...
        if 'color_contrast' in self._configured_rule_names('accessibility'):
        ...
        if 'keyboard_nav' in self._configured_rule_names('accessibility'):
    ...
    def check_seo(self, html: str) -> List[Dict]:
        ...
        if 'meta_description' in self._configured_rule_names('seo'):
        ...
        # Check canonical URL
        if 'canonical_url' in self._configured_rule_names('seo'):
        ...
        # Check valid JSON-LD
        if 'valid_jsonld' in self._configured_rule_names('seo'):
    ...
    def _is_executable_script(self, script) -> bool:
        ...
        return bool(script.get('src')) or script_type in {
            '',
            'text/javascript',
    ...
    def check_budgets(self, html_path: Path) -> List[Dict]:
        ...
        if js_budget == 0:
            ...
            if executable_scripts and not self._is_interactive_utility_page(html_path):
                issues.append({
                    'severity': 'error',
                    'rule': 'js_budget',

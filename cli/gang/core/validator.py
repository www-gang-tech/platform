...
class ContractValidator:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.js_allowed_sections = {'products', 'cart', 'search', 'studio'}
    ...
    def check_budgets(self, html_path: Path) -> List[Dict]:
        ...
        # Data-only scripts such as JSON-LD are required structured data, not
        # executable page JavaScript.
        js_budget = self.budgets.get('js', float('inf'))
        path_parts = set(html_path.parts)
        if js_budget == 0 and not (path_parts & self.js_allowed_sections):

...
class ContractValidator:
    ...
    @staticmethod
    def _is_positive_tabindex(value: str) -> bool:
        """Return True for positive tabindex values without crashing on invalid markup."""
        if not value:
            return False
        try:
            return int(value) > 0
    ...
    def check_budgets(self, html_path: Path) -> List[Dict]:
        ...
        if js_budget == 0:
            ...
            if path_parts.intersection(interactive_sections):
                return issues

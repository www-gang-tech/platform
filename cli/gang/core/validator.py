...
class ContractValidator:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.interactive_sections = {'products', 'cart', 'search', 'studio'}
    ...
    def check_budgets(self, html_path: Path) -> List[Dict]:
        ...
        allows_js = bool(path_parts & self.interactive_sections)
        if js_budget == 0 and not allows_js:
            soup = BeautifulSoup(content, 'html.parser')
            scripts = [
                script for script in soup.find_all('script', src=True)
                if (script.get('type') or '').lower() not in {'application/ld+json', 'application/json'}
            ]
            inline_scripts = [
                script for script in soup.find_all('script', src=False)
                if (script.get('type') or 'text/javascript').lower() not in {'application/ld+json', 'application/json'}
            ]

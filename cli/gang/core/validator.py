...
class ContractValidator:
    ...
    def _rule_names(self, section: str) -> List[str]:
        """Return configured rule names, tolerating empty dict entries."""
        names = []
    ...
    def _allows_executable_js(self, html_path: Path) -> bool:
        """Interactive utility pages can ship JS; read-only content pages cannot."""
        return bool({'cart', 'products', 'search', 'studio'} & set(html_path.parts))
    ...
    def check_accessibility(self, html: str) -> List[Dict]:
        ...
        if 'keyboard_nav' in self._rule_names('accessibility'):
            ...
            for elem in soup.find_all(attrs={'tabindex': True}):
                try:
                    if int(elem.get('tabindex')) > 0:
                        bad_tabindex.append(elem)

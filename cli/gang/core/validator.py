...
class ContractValidator:
    ...
    def _allows_executable_js(self, html_path: Path, soup: BeautifulSoup) -> bool:
    ...
    def _positive_tabindex(self, value) -> bool:
        try:
            return int(value) > 0
    ...
    def check_accessibility(self, html: str) -> List[Dict]:
        ...
        if 'keyboard_nav' in self._rule_names('accessibility'):
            bad_tabindex = soup.find_all(attrs={'tabindex': self._positive_tabindex})
            if bad_tabindex:
                issues.append({
                    'severity': 'warning',
                    'rule': 'keyboard_nav',
    ...
    def check_budgets(self, html_path: Path) -> List[Dict]:
        ...
        if js_budget == 0:
            ...
            if scripts and not self._allows_executable_js(html_path, soup):
                issues.append({
                    'severity': 'error',
                    'rule': 'js_budget',

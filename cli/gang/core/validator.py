...
class ContractValidator:
    ...
    def _rule_enabled(self, section: str, rule_name: str) -> bool:
        """Return whether a named contract rule is enabled."""
    ...
    def _non_executable_script(self, script) -> bool:
        """Data script tags (JSON-LD, import maps, templates) do not consume JS budget."""
    ...
    def _is_interactive_utility_page(self, html_path: Path) -> bool:
        """Known utility pages can use JS while content pages remain zero-JS."""
        try:
            dist_path = Path(self.config['build']['output'])
        ...
        return relative_path.parts[0] in {'cart', 'products', 'search', 'studio'}
    ...
    def check_accessibility(self, html: str) -> List[Dict]:
        ...
        if self._rule_enabled('accessibility', 'color_contrast'):
        ...
        if self._rule_enabled('accessibility', 'keyboard_nav'):
    ...
    def check_seo(self, html: str) -> List[Dict]:
        ...
        if self._rule_enabled('seo', 'meta_description'):
        ...
        # Check canonical URL
        if self._rule_enabled('seo', 'canonical_url'):
        ...
        # Check valid JSON-LD
        if self._rule_enabled('seo', 'valid_jsonld'):
    ...
    def check_budgets(self, html_path: Path) -> List[Dict]:
        ...
        if js_budget == 0 and not self._is_interactive_utility_page(html_path):
            ...
            scripts = [script for script in soup.find_all('script', src=True)
                       if not self._non_executable_script(script)]

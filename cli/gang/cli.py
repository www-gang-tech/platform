...
def _has_real_comments_webhook(config: Dict) -> bool:
    """Return True only when comments have a usable, non-placeholder webhook."""
...
def _text_or_fallback(value, fallback: str) -> str:
    text = str(value).strip() if value is not None else ''
    return text or fallback


def _json_safe_date(value) -> str:
    if hasattr(value, 'isoformat'):
        return value.isoformat()
    return str(value).strip() if value is not None else ''


def _default_jsonld(content_type: str, title: str, description: str, canonical_url: str, date, site: Dict) -> Dict:
    ...
    if content_type in ('posts', 'articles'):
        return {
            ...
            '@type': 'BlogPosting',
            'headline': title,
            'datePublished': _json_safe_date(date) or datetime.now().date().isoformat(),
            'author': organization,
    ...
    if content_type == 'projects':
        return {
            ...
            'description': description,
            'author': organization,
            'dateCreated': _json_safe_date(date) or datetime.now().date().isoformat(),
            'url': canonical_url,
        }
...
def generate_agentmap(ctx):
    ...
    # Get publishable content from the same first-class content types the build renders.
    scheduler = ContentScheduler(content_path)
...
def build(ctx, check_quality, min_quality_score, validate_links, check_slugs, optimize_images, profile):
    ...
    for md_file in publishable_files:
        ...
        title = _text_or_fallback(frontmatter.get('title'), md_file.stem.replace('-', ' ').title())
        description = _text_or_fallback(
            seo.get('description') or frontmatter.get('summary') or frontmatter.get('description'),
        ...
        comments_enabled = _has_real_comments_webhook(config)
        ...
        context = {
            ...
            'title': title,
            'description': description,
            'summary': frontmatter.get('summary', description),
            ...
            'jsonld': frontmatter.get('jsonld') or None,
    ...
    if all_people:
        ...
        for person in all_people:
            people_by_section.setdefault(person.get('section') or 'People', []).append(person)
        people_html = template_engine.render('people-list.html', {
            'site_title': config['site']['title'],
            'lang': config['site']['language'],
            'title': 'People',
            'description': 'People connected to GANG.',

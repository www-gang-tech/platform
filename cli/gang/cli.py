...
def check_contracts(ctx, verbose):
    """Validate site against page-type contracts and standards"""
    try:
    ...
    # Map dist paths to content types. Collection index pages are checked by
    # the general validator; type contracts apply to detail pages.
    type_mapping = {
        'posts': 'post',
        'pages': 'page',
        'projects': 'project',
...
def has_usable_comments_config(config: Dict) -> bool:
    """Return true only when comments have a real submission endpoint."""
...
def get_content_description(frontmatter: Dict, config: Dict) -> str:
    """Prefer authored SEO/summary text, falling back to the site description."""
...
def strip_leading_markdown_h1(body: str) -> str:
    """Remove one leading markdown H1 when the selected template owns the H1."""
    return re.sub(r'^\s*#\s+.+?(?:\r?\n)+', '', body, count=1)


def default_jsonld(frontmatter: Dict, content_type: str, canonical_url: str, config: Dict, slug: str) -> Dict:
    """Generate contract-compliant JSON-LD when frontmatter leaves it empty."""
    ...
    if content_type == 'people':
        return {
            ...
            'url': canonical_url,
            'description': description,
            'jobTitle': frontmatter.get('role', frontmatter.get('jobTitle', '')),
        }
...
def build(ctx, check_quality, min_quality_score, validate_links, check_slugs, optimize_images, profile):
    ...
    for md_file in publishable_files:
        ...
        # Convert markdown to HTML. Detail templates below render their own H1,
        # so strip only the first leading markdown H1 for those page types.
        body_for_template = body
        if content_type in ('posts', 'projects', 'newsletters', 'people'):
            body_for_template = strip_leading_markdown_h1(body)
        md = markdown.Markdown(extensions=['extra', 'meta'])
        content_html = md.convert(body_for_template)
        ...
        description = get_content_description(frontmatter, config)
        comments_enabled = has_usable_comments_config(config)
        ...
        context = {
            ...
            'jsonld': default_jsonld(frontmatter, content_type, canonical_url, config, slug),
            'canonical_url': canonical_url,
            'og_type': 'article' if content_type in ('posts', 'projects', 'newsletters') else 'profile' if content_type == 'people' else 'website',
            'twitter_card': 'summary',
            'comments': [],
            'comments_enabled': comments_enabled,
            'comments_webhook_url': config.get('comments', {}).get('webhook_url', ''),
            'role': frontmatter.get('role') or frontmatter.get('jobTitle'),
...
def create_list_page_simple(config: Dict, items: List, title: str, templates_path: Path = None) -> str:
    ...
    canonical_url = f"{config['site']['url']}/{collection_slug}/"
    jsonld = {
        "@context": "https://schema.org",
        "@type": "CollectionPage",
        "name": title,
...
def check(ctx, output):
    ...
    for file_result in results['files']:
        ...
        if not file_summary['passed']:
            html_file = Path(file_result['file'])
            display_path = html_file.relative_to(dist_path) if html_file.is_relative_to(dist_path) else html_file
            click.echo(f"\n❌ {display_path}")
